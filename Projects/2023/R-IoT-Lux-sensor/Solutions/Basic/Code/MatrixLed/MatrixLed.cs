...
namespace MyLib
{
    ...
    public class SimpleHT16K33
    {
        ...
        public void ShowMatrixAsynch(ulong matrix8x8, bool bScrollLastCharacters = true, int msPerLetter = 50, ColorMode colorMode = ColorMode.Default, bool Circulate = true)
        {
            int lastPixel = 8 + 1;
            if (!bScrollLastCharacters)
                lastPixel -= 15;
            ...
            var task = new Thread(() =>
            {
                writeDisplay(BitConverter.GetBytes(matrix8x8), cursorEnd: lastPixel, scrollingTimeInMs: msPerLetter, bCirculate : Circulate, colorMode: colorMode);
                are.Set();
            })
            { Priority = ThreadPriority.BelowNormal };
            task.Start();
            if(!Circulate)
            {
                Interlocked.Exchange(ref noCirculate, 0);
            }
